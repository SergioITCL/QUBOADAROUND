...
class QUBOAnnealer(IRoundOptimizer):
    ...
    def _qubo_dense(self, tensors: List[np.ndarray]) -> List[np.ndarray]:
        ...
        '''
        n=150
        kernel = np.random.uniform( -1,1, size=(10,n))
        bias = np.random.uniform( -1,1, size=(10,))
        #input_f = np.random.exponential(scale=10.0, size=(1000, n))
        input_f = np.random.normal(0,1,size=(1000, n))
        ...
        '''
        input_f = np.random.uniform(0.5, size=(1000, 49))

        #input_f=x_test=np.load('X_test.npy') 
        print(input_f.shape)
        input_f=input_f[:1000]
        kernel = np.random.uniform(-1, 1, size=(10,49))
        bias = np.random.uniform( -1,1, size=(10,))
        output_f = np.zeros((input_f.shape[0], bias.shape[0]))
        for i in range(0,input_f[:,0].shape[0]):
            output_f[i]=kernel.dot(input_f[i])+bias
        ...
        for l in range(0,3):
            ...
            print("Su valor absoluto es:", abs(diccionarioM[max_abs_value_key]))
            fuerza=abs(diccionarioM[max_abs_value_key])
            mi_diccionario = diccionarioM
            # Nombre del archivo CSV
            nombre_archivo = 'normal7x7qubodiccionario'
            # Lista de nombres de campo (encabezados)
            campos = mi_diccionario.keys()
            # Lista de valores
            ...
            with open(nombre_archivo, 'w', newline='') as archivo:
                ...
                writer.writeheader()
                writer.writerow(mi_diccionario)
        ...
        plt.title('Distribución de coeficientes Mnist - Fashion')
        plt.xlabel('Valor')
        ...
        valores = list(DiccionarioT.values())
 
        min_valor = -0.00006
        max_valor = 0.00029
        
        num_intervalos = 50  # Puedes ajustar el número de intervalos según tus necesidades
        ...
        plt.xticks(np.round(intervalos, 5), rotation=45)
        plt.grid(True)
        plt.show()
        '''
