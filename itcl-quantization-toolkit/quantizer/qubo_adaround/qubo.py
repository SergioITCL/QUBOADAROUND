...
from qubo_adaround.qubocalculation import *
...
class QUBOAnnealer(IRoundOptimizer):
    ...
    def __init__(
        ...
        quantization_to_round_nearest : bool = False,
        ...
        self.quantization_to_round_nearest = quantization_to_round_nearest
    ...
    def _qubo_dense(self, tensors: List[np.ndarray]) -> List[np.ndarray]:
        ...
        """
        if self.quantization_to_round_nearest is True:
            return tensors


        if self.random_qubo_num_variables is False:
        ...
        if self.random_adaround_coefficients is True:
